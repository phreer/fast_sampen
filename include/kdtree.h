...
template <typename T, unsigned K> class KDCountingTreeNode {
public:
  /*
...
template <typename T, unsigned K> class KDCountingTree {
public:
...
template <typename T, unsigned K> class KDCountingTree2KNode {
public:
  /*
   * @brief Construct kd tree nodes recursively.
   *
...
template <typename T, unsigned K> class KDCountingTree2K {
public:
...
template <typename T, unsigned K> class KDTreeNode2K {
public:
...
template <typename T, unsigned K> class KDTree2K {
public:
  KDTree2K(const vector<KDPoint<T, K + 1>> &points, OutputLevel output_level)
    ...
    _root = new KDTreeNode2K<T, K>(0, nullptr, _leaves, _points.begin(),
                                   _points.end(), 0);
    for (unsigned i = 0; i < n; ++i) {
      _index2leaf[_points[i].value()] = i;
    }
...
template <typename T, unsigned K> void KDCountingTree<T, K>::Print() {
  if (_root) {
...
inline long long KDCountingTree<T, K>::CountRange(const Range<T, K> &range,
...
template <typename T, unsigned K> void KDCountingTreeNode<T, K>::Print() {
...
template <typename T, unsigned K>
KDTreeNode2K<T, K>::KDTreeNode2K(
    unsigned depth, KDTreeNode2K *father, vector<KDTreeNode2K *> &leaves,
    typename vector<KDPoint<T, K + 1>>::iterator first,
  ...
  for (unsigned i = 0; i < (1u << K); i++) {
    ...
    splitter2 = splitters[i + 1];
    if (splitter1 != splitter2) {
      KDTreeNode2K<T, K> *child =
          new KDTreeNode2K<T, K>(_depth + 1, this, leaves, first + splitter1,
...
template <typename T, unsigned K>
vector<long long> KDTreeNode2K<T, K>::CountRange(
    const Range<T, K + 1> &range, long long &num_nodes,
    const vector<KDTreeNode2K *> &leaves, vector<const KDTreeNode2K *> &q1,
    vector<const KDTreeNode2K *> &q2) const {
