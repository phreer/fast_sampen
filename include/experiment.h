...
void SampleEntropySamplingExperiment(SampleEntropyCalculatorSampling<T, K> &secds, 
                                     unsigned n_computation)
{
    ...
    for (unsigned i = 0; i < n_computation; ++i)
    {
        ...
        if (n_computation == 1) std::cout << secds.get_result_str();
    ...
    if (n_computation > 1)
    {
        ...
        double mean_computation_time = ComputeSum<double>(computation_times) / n_computation;
        ...
        std::cout << "\tmean_computation_time: " << mean_computation_time << std::endl;
        std::cout << "----------------------------------------"
            << "----------------------------------------\n";
    }
}
