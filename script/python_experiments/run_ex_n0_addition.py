...
output_dir = os.path.join(script_dir, '../../result/convergence_n0_addition', tag)
n1 = 150
output_name = '{db_name}_{record_name}_n0{n0}'
...
def do_convergence_n0_additional(db_name, record_name):
  ...
  for n0 in list_n0:
    def callback(future):
      if future.exception():
        print('error occurred %s %s %d' % (db_name, record_name, n0))
        print(future)
      else:
        print('completed %s %s %d' % (db_name, record_name, n0))
    output_filename = output_name.format(db_name=db_name, record_name=record_name, n0=n0)
    print('submit %s %s %d' % (db_name, record_name, n0))
...
if __name__ == '__main__':
  ...
  for db_name, record_name in list_db_records:
    pool = do_convergence_n0_additional(db_name=db_name, record_name=record_name)
